...
const newCardVariants = cva("w-full relative", {
  variants: {
    variant: {
      default: [
        "border rounded-lg",
...
const EducationCardContent = React.forwardRef(({ className, ...props }, ref) => (
  <div
    ref={ref}
    className={cn("p-6 flex flex-col items-center", className)}
    {...props}
  >
    {props.children}
  </div>
...
const EducationCard = React.forwardRef(
  (
    {
      className,
      variant,
    ...
    const content = (
      <EducationCardContent>
        {degree && (
          <h2 className="text-xl font-bold mb-1 text-gray-900 dark:text-gray-100 underline decoration-slice">
            {degree}
          </h2>
        )}
        {college && (
          <h3 className="text-xl font-bold mb-1 text-gray-900 dark:text-gray-100 pt-4">
            {college}
          </h3>
        )}
        {grades && (
          <h3 className="text-lg font-bold mb-1 text-gray-900 dark:text-gray-100 pt-8">
            {grades}
          </h3>
        )}
        {duration && (
          <h3 className="text-lg font-bold mb-1 text-gray-900 dark:text-gray-100 pt-0.5 pb-3">
            {duration}
          </h3>
        )}
        <div className="border-t-2 w-8/12 pt-3 text-center">
          <ul>
            {courses.map((course) => <li className="pb-2">{course}</li>)}
          </ul>
        </div>
    ...
    if (variant === "dots") {
      return (
        <div
          ref={ref}
          className={cn(newCardVariants({ variant, className }))}
          {...props}
        >
          <div className="absolute left-0 top-4 -z-0 h-px w-full bg-zinc-400 dark:bg-zinc-700 sm:top-6 md:top-8" />
          <div className="absolute bottom-4 left-0 z-0 h-px w-full bg-zinc-400 dark:bg-zinc-700 sm:bottom-6 md:bottom-8" />
...
export { EducationCard, EducationCardContent, newCardVariants };
